...
(for [i (range 20)]
  (= (sort (vals (update-vals m inc)))
   (sort (vals (p/update-vals m inc)))))

(let [m (large-map 1e5)] (quick-bench (update-vals m inc))) ;; 22ms
(let [m (large-map 1e5)] (quick-bench (update-vals-transients m inc))) ;; 15ms
(let [m (large-map 1e5)] (binding [p/*mutable* true] (quick-bench (p/update-vals m inc)))) ;; 16ms
(let [m (large-map 1e5)] (binding [p/*mutable* false] (quick-bench (p/update-vals m inc)))) ;; 56ms
...
(let [m (large-map 1e6)] (binding [p/*mutable* true] (quick-bench (p/update-vals m inc)))) ;; 215ms
(let [m (large-map 1e6)] (binding [p/*mutable* false] (quick-bench (p/update-vals m inc)))) ;; 1.09secs
...
(let [m (large-map 1e5)] (binding [p/*mutable* false] (quick-bench (p/update-vals m f)))) ;; 1.8secs
(let [m (large-map 1e5)] (binding [p/*mutable* true] (quick-bench (p/update-vals m f)))) ;; 1.6secs
